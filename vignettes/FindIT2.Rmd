...
When collecting several samples ATAC data and get a merge peak set, you do
k-means to cluster peak. Then you want to know which TF is enrichment in your
specific peak set compared with the total peak set. In this condition, you can
use the `findIT_enrichInShuffle`, it will sample same set number feature_id as
your `input_feature_id`, then it will calculate the p-value to mark the
significant of specific TF.

The object you may need to consider:

- shuffleN: 

The more you sample, the more accurate the final result. But it will make
function run more time.
...
You can also find the enrichment of TF using `findIT_enrichInAll`, it use the 
same theory like GO-enrich analysis. The background is total ATAC peak, and the 
select set is your `input_feature_id`. Compared with `findIT_enrichInShuffle` 
above, its runs more quickly. But it will have a little problem when using motif
scan result as `TF_GR_database`. A TF may hit more than one time in a peak, 
however, here I treat it as one because I want the whole frame to be more like 
GO enrichment analysis. Actually, the TF hit number can offer some other useful 
information, which you can see in `findIT_MARA`. But it will do not have a big 
effect on the final result. After all, what we really need is TF rank instead of
p-value.
...
In the meanwhile, you can parse your result using `jaccard_findIT_enrichInAll`,
...
The `findIT_TTPair` also use the fisher test like `findIT_enrichInAll`. The
...
`shinyParse_findIT_regionRP` to help you parse and understend the result.
